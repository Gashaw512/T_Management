...
const InboxItems: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleInboxItemsUpdated = (event: CustomEvent<{ count: number, firstItemContent: string }>) => {
      ...
      if (event.detail.count > 0) {
        ...
        showSuccessToast(`New item from Telegram: ${event.detail.firstItemContent}`); // Hardcoded string

        // If multiple new items, show a summary notification as well
        if (event.detail.count > 1) {
          showSuccessToast(`${event.detail.count - 1} more new items added`); // Hardcoded string
        }
      }
    };
  ...
  const handleProcessItem = async (id: number) => {
    try {
      ...
      showSuccessToast('Inbox item processed!'); // Hardcoded string
    } catch (error) {
      console.error('Failed to process inbox item:', error);
      showErrorToast('Failed to process inbox item.'); // Hardcoded string
    }
  };
  ...
  const handleSaveEditedItem = async (text: string) => {
    try {
      if (itemToEdit !== null) {
        ...
        showSuccessToast('Inbox item updated!'); // Hardcoded string
      }
      setIsEditModalOpen(false);
      setItemToEdit(null);
    } catch (error) {
      console.error('Failed to update inbox item:', error);
      showErrorToast('Failed to update inbox item.'); // Hardcoded string
    }
  };
  ...
  const handleDeleteItem = async (id: number) => {
    try {
      ...
      showSuccessToast('Inbox item deleted!'); // Hardcoded string
    } catch (error) {
      console.error('Failed to delete inbox item:', error);
      showErrorToast('Failed to delete inbox item.'); // Hardcoded string
    }
  };
  ...
  const handleOpenTaskModal = async (task: Task, inboxItemId?: number) => {
    ...
    try {
      ...
      showErrorToast('Failed to load projects.'); // Hardcoded string
  ...
  const handleSaveTask = async (task: Task) => {
    try {
      ...
      const taskLink = (
        <span>
          Task <a href={`/task/${createdTask.uuid}`} className="text-green-200 underline hover:text-green-100">{createdTask.name}</a> created successfully! {/* Hardcoded string */}
        </span>
      );
      ...
      showErrorToast('Failed to create task.'); // Hardcoded string
    }
  };
  ...
  const handleSaveProject = async (project: Project) => {
    try {
      ...
      showSuccessToast('Project created successfully!'); // Hardcoded string
      ...
      showErrorToast('Failed to create project.'); // Hardcoded string
    }
  };
  ...
  const handleSaveNote = async (note: Note) => {
    try {
      ...
      showSuccessToast('Note created successfully!'); // Hardcoded string
      ...
      showErrorToast('Failed to create note.'); // Hardcoded string
    }
  };
  ...
  if (inboxItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 space-y-4 text-center text-gray-600 dark:text-gray-300">
        ...
        <h3 className="text-xl font-semibold">Inbox is empty!</h3> {/* Hardcoded string */}
        <p>Your inbox is clear. Add new items or enjoy your organized life!</p> {/* Hardcoded string */}
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-2xl font-light">Inbox</h1> {/* Hardcoded string */}
      </div>

      <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
        Inbox is where all uncategorized tasks are located. Tasks that have not been assigned to a project or don't have a due date will appear here. This is your 'brain dump' area where you can quickly note down tasks and organize them later. {/* Hardcoded string */}
      </p>
