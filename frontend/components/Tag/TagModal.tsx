...
const TagModal: React.FC<TagModalProps> = ({
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim()) {
      showErrorToast('Tag name is required.'); // Hardcoded string
      return;
    }
    ...
    try {
      if (tag) {
        showSuccessToast('Tag updated successfully!'); // Hardcoded string
      } else {
        showSuccessToast('Tag created successfully!'); // Hardcoded string
      }
      onSave(formData);
      handleClose();
    } catch (err) {
      showErrorToast('Failed to save tag.'); // Hardcoded string
    } finally {
              ...
              <div className="p-4 space-y-3 flex-1 text-sm overflow-y-auto">
                ...
                <div className="py-4">
                  <input
                    ...
                    placeholder="Enter tag name" // Hardcoded string
                  />
                </div>
              </div>
              ...
              <div className="p-3 flex-shrink-0 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-2">
                ...
                >
                  {isSubmitting
                    ...
                    : tag
                    ? 'Update Tag' // Hardcoded string
                    : 'Create Tag'} {/* Hardcoded string */}
                </button>
              </div>
            </fieldset>
          </form>
