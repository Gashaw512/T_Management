    ...
    try {
      ...
      if (saveMode === 'task') {
        ...
        try {
          ...
          showSuccessToast('Task created successfully!'); // Hardcoded string
          setInputText('');
          handleClose();
        } catch (error: any) {
        ...
        try {
          ...
          showSuccessToast('Inbox item added successfully!'); // Hardcoded string

          handleClose();
        } catch (error) {
          console.error('Failed to create inbox item:', error);
          showErrorToast('Failed to add inbox item.'); // Hardcoded string
          setIsSaving(false);
        }
      }
    } catch (error) {
      ...
      if (editMode) {
        showErrorToast('Failed to update item.'); // Hardcoded string
      } else {
        showErrorToast(saveMode === 'task' ? 'Failed to create task.' : 'Failed to add inbox item.'); // Hardcoded strings
      }
    } finally {
      setIsSaving(false);
    }
  }, [inputText, isSaving, editMode, onEdit, saveMode, onSave, showSuccessToast, showErrorToast, onClose, handleClose]);
          ...
          <div className="w-full p-6 px-8">
            <div className="flex flex-col sm:flex-row sm:items-center">
              <input
                ...
                placeholder="Capture a thought or task..." // Hardcoded string
                onKeyDown={(e) => {
              ...
              >
                {isSaving ? 'Saving...' : 'Save'} {/* Hardcoded strings */}
              </button>
            </div>
