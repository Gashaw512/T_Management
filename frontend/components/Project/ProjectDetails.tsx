...
const ProjectDetails: React.FC = () => {
  ...
  const handleTaskCreate = async (taskName: string) => {
    ...
    try {
      ...
      const taskLink = (
        <span>
          Task <a href={`/task/${newTask.uuid}`} className="text-green-200 underline hover:text-green-100">{newTask.name}</a> created successfully! {/* Hardcoded string */}
        </span>
      );
  ...
  const formatProjectDueDate = (dateString: string) => {
    ...
    // Replaced i18n.language with a hardcoded locale 'en-US'
      ...
      <div className="w-full max-w-5xl">
        ...
        {!showAutoSuggestForm && (
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Tasks</h3> {/* Hardcoded string */}
