...
const AreaModal: React.FC<AreaModalProps> = ({ isOpen, onClose, area, onSave }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim()) {
      setError('Area name is required.'); // Hardcoded string
      return;
    }
    ...
    try {
      ...
      showSuccessToast(formData.id ? 'Area updated successfully!' : 'Area created successfully!'); // Hardcoded strings
      handleClose();
    } catch (err) {
      setError((err as Error).message);
      showErrorToast('Failed to save area.'); // Hardcoded string
    } finally {
        ...
        <form className="flex flex-col flex-1">
          <fieldset className="flex flex-col flex-1">
            <div className="p-4 space-y-3 flex-1 text-sm overflow-y-auto">
              ...
              <div className="py-4">
                <input
                  ...
                  placeholder="Area Name" // Hardcoded string
                />
              </div>
              ...
              <div className="pb-3">
                ...
                <textarea
                  ...
                  placeholder="Enter a brief description for this area" // Hardcoded string
                />
              </div>
            ...
            <div className="p-3 flex-shrink-0 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-2">
              ...
              >
                {isSubmitting
                  ? 'Saving...' // Hardcoded string
                  : formData.id && formData.id !== 0
                  ? 'Update Area' // Hardcoded string
                  : 'Create Area'} {/* Hardcoded string */}
              </button>
            </div>
          </fieldset>
        </form>
