...
const NoteModal: React.FC<NoteModalProps> = ({ isOpen, onClose, note, onSave }) => {
  ...
  useEffect(() => {
    const loadTags = async () => {
      try {
        ...
        showErrorToast('Failed to load tags.'); // Hardcoded string
      }
    };
  ...
  const handleSubmit = async () => {
    if (!formData.title.trim()) {
      setError('Note title is required.'); // Hardcoded string
      return;
    }
    ...
    try {
      ...
      showSuccessToast(formData.id && formData.id !== 0 ? 'Note updated successfully!' : 'Note created successfully!'); // Hardcoded string
      handleClose();
    } catch (err) {
      setError((err as Error).message);
      showErrorToast('Failed to save note.'); // Hardcoded string
    } finally {
        ...
        <form className="flex flex-col flex-1">
          <fieldset className="flex flex-col flex-1">
            <div className="p-4 space-y-3 flex-1 text-sm overflow-y-auto">
              <div className="py-4">
                <input
                  ...
                  placeholder="Note Title" // Hardcoded string
                />
              </div>

              <div className="pb-3">
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Tags {tags.length > 0 ? `(${tags.join(', ')})` : ''} {/* Hardcoded string */}
                </label>
              ...
              <div className="pb-3 flex-1">
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">
                    Note Content <span className="text-gray-500">(Markdown supported)</span> {/* Hardcoded string */}
                  </label>
            ...
            <div className="p-3 flex-shrink-0 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-2">
              ...
              >
                {isSubmitting
                  ? 'Submitting...' // Hardcoded string
                  : formData.id && formData.id !== 0
                  ? 'Update Note' // Hardcoded string
                  : 'Create Note'} {/* Hardcoded string */}
              </button>
            </div>
          </fieldset>
        </form>
