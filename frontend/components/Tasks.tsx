...
// Helper function to get search placeholder - now always returns English
...
const Tasks: React.FC = () => {
  ...
  // Note: getTitleAndIcon and getDescription might still be using `t` internally.
  // You'll need to check those files as well if they are still causing issues.
  // For now, passing a dummy 't' function if it's required by those utils.
  // Or, ideally, modify getTitleAndIcon and getDescription to return static strings if you fully remove i18n.
  // For this example, I'm assuming those utils will be updated or can handle a missing 't'.
  const dummyT = (_key: string, fallback: string) => fallback; // A simple placeholder
  const { title, icon } =
    stateTitle && stateIcon
      ? { title: stateTitle, icon: stateIcon }
      : getTitleAndIcon(query, projects, dummyT); // Pass dummyT
  ...
  const handleTaskCreate = async (taskData: Partial<Task>) => {
    try {
      ...
      // Show success toast with task link - using hardcoded strings now
      const taskLink = (
        <span>
          Task <a href={`/task/${newTask.uuid}`} className="text-green-200 underline hover:text-green-100">{newTask.name}</a> created successfully!
        </span>
      );
  ...
  const description = getDescription(query, projects, dummyT); // Pass dummyT
  ...
  const getSortDisplayName = (order: string) => {
    ...
    return field.split("_").map(capitalize).join(" ");
  };
      ...
      <div className="w-full max-w-5xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4">
          ...
          <div className="relative inline-block text-left" ref={dropdownRef}>
            ...
            >
              ...
              {getSortDisplayName(orderBy)} {/* Use helper for display name */}
                ...
                <div className="py-1 max-h-60 overflow-y-auto" role="none">
                  {[
                    ...
                    <button
                      ...
                      role="menuitem"
                    >
                      {getSortDisplayName(order)} {/* Use helper for display name */}
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div className="mb-4">
          <div className="flex items-center bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm p-2">
            ...
            <input
              type="text"
              placeholder={getSearchPlaceholder()} // No longer needs i18n.language
          ...
          <>
            ...
            {filteredTasks.length > 0 ? (
              ...
              <p className="text-gray-500 text-center mt-4">
                No tasks available. {/* Hardcoded "No tasks" message */}
              </p>
            )}
          </>
        )}
