...
const PriorityDropdown: React.FC<PriorityDropdownProps> = ({ value, onChange }) => {
  ...
  const priorities = [
    { value: 'low', label: 'Low', icon: <ArrowDownIcon className="w-5 h-5 text-gray-700 dark:text-gray-300" /> }, // Hardcoded string
    { value: 'medium', label: 'Medium', icon: <ArrowUpIcon className="w-5 h-5 text-gray-700 dark:text-gray-300" /> }, // Hardcoded string
    { value: 'high', label: 'High', icon: <FireIcon className="w-5 h-5 text-gray-700 dark:text-gray-300" /> } // Hardcoded string
  ];
    ...
    <div ref={dropdownRef} className="relative inline-block text-left w-full">
      ...
      >
        <span className="flex items-center space-x-2">
          ...
          <span>{selectedPriority ? selectedPriority.label : 'Select Priority'}</span> {/* Hardcoded string */}
        </span>
