    ...
    <div className={`relative flex items-center space-x-2 ${className}`}>
      <div className="flex items-center space-x-2 bg-gray-100 dark:bg-gray-800 rounded-lg px-3 py-1">
        ...
        <button
          ...
          aria-label={isRunning ? 'Pause Pomodoro Timer' : 'Play Pomodoro Timer'} // Hardcoded string
        >
          {isRunning ? (
        ...
        <button
          ...
          aria-label="Reset Pomodoro Timer" // Hardcoded string
        >
        ...
        <button
          ...
          aria-label="Close Pomodoro Timer" // Hardcoded string
        >
      ...
      {showCompletionMessage && (
        <div className="absolute top-full mt-2 right-0 bg-green-100 dark:bg-green-900 border border-green-300 dark:border-green-700 text-green-800 dark:text-green-200 px-3 py-2 rounded-lg shadow-lg z-50 whitespace-nowrap">
          <div className="flex items-center space-x-2 mb-2">
            <span className="text-sm font-medium">🍅 Pomodoro Complete!</span> {/* Hardcoded string */}
          </div>
          <p className="text-xs mb-3">Time for a short break.</p> {/* Hardcoded string */}
          <button
            onClick={() => {
              setShowCompletionMessage(false);
              setIsActive(false);
